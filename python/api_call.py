import sys
import requests
from PyQt5.QtWidgets import QApplication, QWidget, QVBoxLayout, QHBoxLayout, QTableWidget, QTableWidgetItem, QPushButton, QLineEdit, QLabel, QMessageBox
...
class POSWindow(QWidget):
    ...
    def initUI(self):
        ...
        self.price_input.setDisabled(True)  # Disable direct editing of price
    ...
    def addItem(self):
        ...
        if not item or not quantity:
            QMessageBox.warning(self, 'Input Error', 'Item and Quantity are required!')
            return
        ...
        price = self.fetchItemPrice(item)
        if price is not None:
            ...
            self.table.setItem(row_position, 2, QTableWidgetItem(str(price * quantity)))

            self.item_input.clear()
            self.quantity_input.clear()
            self.price_input.clear()
        else:
            QMessageBox.warning(self, 'API Error', 'Could not fetch item price from API!')

    def fetchItemPrice(self, item):
        # Replace with your actual API endpoint
        api_url = f"https://jsonplaceholder.typicode.com/todos/{item}"
        try:
